...
<title>MB Finance · Exportar Leads</title>
<link href="https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">
<style>
:root{
  --bg:#0B0F19;--surface:#111827;--surface2:#1A2035;--surface3:#1F2937;
  --border:rgba(0,102,204,.12);--border2:rgba(0,102,204,.22);--border-solid:#1E3044;
  --accent:#0066CC;--accent-l:rgba(0,102,204,.08);--accent-d:#004FA3;--accent-glow:rgba(0,102,204,.15);
  --green:#00A651;--green-l:rgba(0,166,81,.08);--green-d:#007A3D;
  --text:#E5E7EB;--text-bright:#F9FAFB;--muted:#64748B;--muted2:#475569;
  --danger:#EF4444;--danger-l:rgba(239,68,68,.08);
  --warn:#F59E0B;--warn-l:rgba(245,158,11,.08);
...
.sidebar-brand{padding:20px;border-bottom:1px solid var(--border-solid)}
.sidebar-brand h1{font-family:var(--font);font-size:18px;font-weight:800;letter-spacing:-.02em}
.sidebar-brand h1 .brand-mb{color:var(--accent)}
.sidebar-brand h1 .brand-fin{color:var(--text)}
.sidebar-brand p{font-size:10px;color:var(--muted);margin-top:3px;font-weight:500;letter-spacing:.06em;text-transform:uppercase}
.nav-group{padding:16px 0 4px}
.nav-label{padding:0 16px 6px;font-size:9px;font-weight:600;color:var(--muted2);text-transform:uppercase;letter-spacing:1.4px;display:block}
.nav-btn{
  display:flex;align-items:center;gap:9px;padding:9px 16px;font-size:12.5px;font-weight:500;
  color:var(--muted);cursor:pointer;border:none;background:none;width:100%;
  text-align:left;transition:all .15s;position:relative;font-family:var(--font);
}
.nav-btn:hover{color:var(--text);background:var(--surface2)}
.nav-btn.active{color:var(--accent);background:var(--accent-l);font-weight:600}
.nav-btn.active::before{content:'';position:absolute;left:0;top:4px;bottom:4px;width:3px;background:var(--accent);border-radius:0 2px 2px 0}
.nav-icon{width:16px;height:16px;border-radius:4px;background:currentColor;opacity:.4;flex-shrink:0}
.sidebar-footer{margin-top:auto;padding:16px;border-top:1px solid var(--border-solid)}
...
.nav-btn.logout-btn{color:var(--danger);font-size:12px;margin-top:4px}
.nav-btn.logout-btn:hover{background:var(--danger-l)}
.sidebar-version{font-size:10px;color:var(--muted2);margin-top:10px}

/* ─── MAIN ─── */
.main{padding:28px 32px;width:100%;box-sizing:border-box;min-height:100vh}
.page{display:none !important}.page.active{display:block !important}
.page-title{font-size:20px;font-weight:700;margin-bottom:3px;letter-spacing:-.02em;color:var(--text-bright)}
...
.section-title{
  ...
  color:var(--muted);margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid var(--border-solid);
}
...
/* ─── FORMS ─── */
.field-label{display:block;font-size:10px;font-weight:600;color:var(--muted);margin-bottom:5px;text-transform:uppercase;letter-spacing:.5px}
input[type=text],input[type=number],select:not([multiple]){
  width:100%;padding:9px 12px;border:1px solid var(--border-solid);border-radius:var(--radius);
  font-size:13px;font-family:var(--font);background:var(--surface2);color:var(--text);
...
.btn{
  ...
  font-size:12px;font-weight:600;border:1px solid transparent;cursor:pointer;
  font-family:var(--font);transition:all .15s;
}
.btn-primary{background:var(--accent);color:#fff;border-color:var(--accent)}
.btn-primary:hover{background:var(--accent-d);box-shadow:0 0 20px var(--accent-glow)}
.btn-secondary{background:var(--surface2);color:var(--text);border-color:var(--border-solid)}
.btn-secondary:hover{border-color:var(--border2);background:var(--surface3)}
.btn-lg{padding:11px 28px;font-size:13px}
...
.cb-item{
  ...
  cursor:pointer;transition:border-color .15s;font-size:12px;background:var(--surface2);
}
.cb-item:hover{border-color:var(--accent)}.cb-item input{accent-color:var(--accent);cursor:pointer;width:14px;height:14px}
.cb-code{font-family:var(--mono);font-size:10px;color:var(--green);margin-left:auto}

/* ─── TAGS ─── */
.quick-tags{display:flex;flex-wrap:wrap;gap:6px}
.qtag{
  padding:3px 10px;border:1px solid var(--border-solid);border-radius:20px;
  font-size:10px;color:var(--muted);cursor:pointer;transition:all .15s;background:var(--surface2);
}
...
.toggle-opt{
  padding:7px 14px;border:1px solid var(--border-solid);font-size:11px;
  cursor:pointer;transition:all .15s;background:var(--surface2);color:var(--muted);font-family:var(--font);
}
.toggle-opt:first-child{border-radius:var(--radius) 0 0 var(--radius)}
.toggle-opt:last-child{border-radius:0 var(--radius) var(--radius) 0;border-left:0}
.toggle-opt.on{background:var(--accent-l);color:var(--accent);border-color:var(--accent);font-weight:600}
.toggle-opt input{display:none}

/* ─── SPLIT ─── */
.split-box{background:var(--warn-l);border:1px solid rgba(245,158,11,.2);border-radius:var(--radius);padding:14px;margin-top:8px}
.split-toggle{position:relative;width:36px;height:20px;cursor:pointer}
.split-toggle input{display:none}
.split-track{position:absolute;inset:0;background:var(--surface3);border-radius:10px;transition:.2s}
.split-thumb{position:absolute;top:2px;left:2px;width:16px;height:16px;background:#fff;border-radius:50%;transition:.2s;box-shadow:0 1px 3px rgba(0,0,0,.3)}
...
.fmt-radio{
  ...
  border:2px solid var(--border-solid);border-radius:var(--radius-lg);
  cursor:pointer;transition:all .15s;background:var(--surface);
}
.fmt-radio:hover{border-color:var(--border2)}
.fmt-radio.sel{border-color:var(--accent);background:var(--accent-l)}
.fmt-radio input{accent-color:var(--accent);width:15px;height:15px}
.fmt-radio-name{font-weight:600;font-size:13px;color:var(--text-bright)}
.fmt-radio-desc{font-size:11px;color:var(--muted)}

/* ─── COLUMN ROW ─── */
.col-row{
  display:grid;grid-template-columns:28px 1fr 1.2fr 1fr 32px;gap:8px;align-items:center;
  background:var(--surface2);border:1px solid var(--border-solid);border-radius:var(--radius);
  padding:6px 8px;margin-bottom:6px;
}
.col-num{font-family:var(--mono);font-size:10px;color:var(--muted2);text-align:center}
...
/* ─── DASHBOARD ─── */
.dash-card{
  background:var(--surface);border:1px solid var(--border-solid);border-radius:var(--radius-lg);
...
.dash-num{font-size:26px;font-weight:700;letter-spacing:-1px;color:var(--text-bright);font-family:var(--mono)}
.dash-lbl{font-size:11px;color:var(--muted)}.dash-pct{font-size:11px;font-weight:500}
...
.bar-track{height:8px;background:var(--surface3);border-radius:4px;overflow:hidden;border:1px solid var(--border-solid)}
.bar-fill{height:100%;border-radius:4px;transition:width .8s cubic-bezier(.4,0,.2,1)}

/* ─── API OVERLAY ─── */
.ainp{background:rgba(0,102,204,.06);border:1px solid rgba(0,102,204,.2);color:var(--accent);padding:7px 10px;border-radius:var(--radius);font-family:var(--mono);font-size:11px;outline:none;box-sizing:border-box}
.ainp:focus{border-color:var(--accent);box-shadow:0 0 0 3px var(--accent-glow)}.ainp option{background:var(--surface);color:var(--accent)}
.albl{color:rgba(0,102,204,.4);font-size:9px;margin-bottom:4px;letter-spacing:1px}
.atag{padding:3px 10px;border:1px solid rgba(0,102,204,.2);border-radius:20px;font-size:9px;color:rgba(0,102,204,.55);cursor:pointer;transition:all .15s}
.atag:hover{border-color:var(--accent);color:var(--accent)}
.preset-btn{
  width:100%;padding:9px 12px;background:rgba(0,102,204,.04);border:1px solid rgba(0,102,204,.15);
  color:rgba(0,102,204,.7);font-family:var(--mono);font-size:10.5px;cursor:pointer;
  border-radius:var(--radius);text-align:left;transition:all .15s;
}
.preset-btn:hover{background:rgba(0,102,204,.1);border-color:rgba(0,102,204,.4);color:var(--accent)}
.preset-btn.active-preset{background:rgba(0,102,204,.12);border-color:var(--accent);color:var(--accent);box-shadow:inset 3px 0 0 var(--accent)}
.aopt-item{display:flex;align-items:flex-start;gap:9px;cursor:pointer;padding:8px 10px;border:1px solid rgba(0,102,204,.1);border-radius:var(--radius);transition:border-color .15s}
.aopt-item:hover{border-color:rgba(0,102,204,.3)}
.aopt-title{font-size:10.5px;color:rgba(0,102,204,.8);font-weight:500}
.aopt-desc{font-size:9px;color:rgba(0,102,204,.35);margin-top:2px;line-height:1.4}
.astat{background:rgba(0,0,0,.3);border:1px solid var(--border-solid);border-radius:var(--radius);padding:10px 14px}
.astat-lbl{font-size:9px;margin-bottom:3px;letter-spacing:1px;color:var(--muted)}
.astat-val{font-size:24px;font-weight:600;font-family:var(--mono)}
@keyframes pulse-dot{0%,100%{opacity:1;box-shadow:0 0 0 0 rgba(0,166,81,.4)}70%{box-shadow:0 0 0 6px rgba(0,166,81,0)}}

/* ─── PRESETS ─── */
.preset-card{
  display:flex;flex-direction:column;align-items:center;gap:4px;padding:14px 8px;
  background:var(--surface);border:2px solid var(--border-solid);border-radius:var(--radius-lg);
  cursor:pointer;transition:all .15s;font-family:var(--font);
}
.preset-card:hover{border-color:var(--accent);background:var(--accent-l);transform:translateY(-2px);box-shadow:0 4px 16px var(--accent-glow)}
.preset-card.active-preset{border-color:var(--accent);background:var(--accent-l);box-shadow:0 0 0 3px var(--accent-glow)}
.preset-card-title{font-size:12px;font-weight:600;color:var(--text-bright)}
...
.aopt-row{
  ...
  border:1px solid var(--border-solid);border-radius:var(--radius-lg);
  transition:border-color .15s;background:var(--surface);
}
.aopt-row:hover{border-color:var(--accent)}
@keyframes apidot{0%,100%{opacity:1;box-shadow:0 0 7px var(--accent)}50%{opacity:.3;box-shadow:none}}
...
/* ─── LIMPEZA OPT HIGHLIGHT ─── */
.limpeza-opt{background:var(--surface);transition:border-color .15s,background .15s}

/* ─── LIMPEZA TOGGLE CARDS ─── */
.limp-card{
...
.limp-card.on{border-color:var(--limp-color,var(--accent))}
.limp-card.on::before{opacity:1;background:var(--limp-color,var(--accent))}
.limp-card:hover{border-color:var(--border2);transform:translateY(-1px);box-shadow:0 4px 16px rgba(0,0,0,.2)}
.limp-card.on:hover{border-color:var(--limp-color,var(--accent))}
.limp-header{display:flex;align-items:center;justify-content:space-between}
.limp-left{display:flex;align-items:center;gap:10px}
.limp-icon{
  width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;
  font-size:16px;background:var(--surface2);transition:background .25s;
}
.limp-card.on .limp-icon{background:color-mix(in srgb,var(--limp-color) 15%,transparent)}
.limp-name{font-size:13px;font-weight:600;color:var(--text-bright)}
.limp-step{font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);transition:color .25s}
.limp-card.on .limp-step{color:var(--limp-color,var(--accent))}
.limp-desc{font-size:11px;color:var(--muted);line-height:1.55}
.limp-desc code{
  font-family:var(--mono);font-size:10px;padding:1px 5px;
  background:var(--surface2);border:1px solid var(--border-solid);border-radius:4px;color:var(--accent);
}
/* ─── TOGGLE SWITCH (generic) ─── */
.tgl-switch{
  position:relative;width:40px;height:22px;flex-shrink:0;cursor:pointer;
}
.tgl-switch input{position:absolute;opacity:0;width:0;height:0;pointer-events:none}
.tgl-track{
  position:absolute;inset:0;border-radius:12px;
  background:var(--surface3);border:1.5px solid var(--border-solid);
...
.tgl-switch input:checked~.tgl-track{
  background:var(--tgl-color,var(--accent));border-color:var(--tgl-color,var(--accent));
}
.tgl-switch input:checked~.tgl-thumb{
  left:21px;
  box-shadow:0 1px 6px rgba(0,0,0,.25),0 0 8px color-mix(in srgb,var(--tgl-color,var(--accent)) 30%,transparent);
}
/* Pipeline master toggle */
.pipeline-toggle{
  display:flex;align-items:center;gap:10px;padding:6px 14px;
  border:1.5px solid var(--accent);border-radius:20px;
  background:var(--accent-l);cursor:pointer;transition:all .2s;
}
.pipeline-toggle:hover{background:color-mix(in srgb,var(--accent) 12%,transparent)}
.pipeline-toggle .tgl-switch{width:36px;height:20px}
.pipeline-toggle .tgl-thumb{top:2px;left:2px;width:16px;height:16px}
.pipeline-toggle .tgl-switch input:checked~.tgl-thumb{left:18px}
.pipeline-label{font-size:12px;font-weight:600;color:var(--accent)}

/* ─── THEME TOGGLE ─── */
.theme-toggle{display:flex;align-items:center;gap:9px;padding:8px 0;margin-bottom:6px;margin-top:6px}
.theme-toggle-label{font-size:11px;font-weight:500;color:var(--muted)}
.theme-switch{
  position:relative;width:36px;height:20px;background:var(--surface3);
  border-radius:20px;cursor:pointer;transition:background .25s;flex-shrink:0;
  border:1.5px solid var(--border-solid);padding:0;
}
...
.theme-switch.active .theme-switch-knob{transform:translateX(16px)}
.theme-switch-knob svg{width:9px;height:9px;transition:opacity .2s}
.theme-sun{color:#F59E0B;opacity:1}
.theme-moon{color:#6366F1;opacity:0;position:absolute}
.theme-switch.active .theme-sun{opacity:0}
.theme-switch.active .theme-moon{opacity:1}

/* ═══════════════════════════════════════════════════════
   LIGHT MODE
   ═══════════════════════════════════════════════════════ */
[data-theme="light"]{
  --bg:#F4F6FA;--surface:#FFFFFF;--surface2:#F0F2F5;--surface3:#E8EBF0;
  --border:rgba(0,0,0,.06);--border2:rgba(0,0,0,.12);--border-solid:#DDE1E8;
  --accent:#0066CC;--accent-l:rgba(0,102,204,.06);--accent-d:#004FA3;--accent-glow:rgba(0,102,204,.12);
  --green:#00A651;--green-l:rgba(0,166,81,.06);--green-d:#007A3D;
  --text:#1E293B;--text-bright:#0F172A;--muted:#64748B;--muted2:#94A3B8;
  --danger:#EF4444;--danger-l:rgba(239,68,68,.06);
  --warn:#F59E0B;--warn-l:rgba(245,158,11,.06);
  --shadow-sm:0 1px 3px rgba(0,0,0,.06);--shadow-md:0 4px 16px rgba(0,0,0,.08);
}
[data-theme="light"] .sidebar{background:#FFFFFF;border-right-color:#E2E6EC}
[data-theme="light"] .sidebar-brand{border-bottom-color:#E2E6EC}
[data-theme="light"] .sidebar-footer{border-top-color:#E2E6EC}
[data-theme="light"] .card{border-color:#E2E6EC;box-shadow:0 1px 4px rgba(0,0,0,.04)}
[data-theme="light"] .card:hover{border-color:#C8D0DA}
[data-theme="light"] input[type=text],[data-theme="light"] input[type=number],[data-theme="light"] select:not([multiple]){
  background:#F8F9FB;border-color:#DDE1E8;color:#1E293B;
}
[data-theme="light"] input:focus,[data-theme="light"] select:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(0,102,204,.1)}
[data-theme="light"] .btn-secondary{background:#F0F2F5;border-color:#DDE1E8;color:#334155}
[data-theme="light"] .btn-secondary:hover{background:#E2E6EC;border-color:#C8D0DA}
[data-theme="light"] .cb-item{border-color:#DDE1E8;background:#F8F9FB}
[data-theme="light"] .qtag{border-color:#DDE1E8;background:#FFFFFF}
[data-theme="light"] .toggle-opt{border-color:#DDE1E8;background:#F0F2F5;color:#64748B}
[data-theme="light"] .limp-card{border-color:#DDE1E8;background:#FFFFFF}
[data-theme="light"] .limp-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.06)}
[data-theme="light"] .limp-icon{background:#F0F2F5}
[data-theme="light"] .limp-desc code{background:#F0F2F5;border-color:#DDE1E8}
[data-theme="light"] .tgl-track{background:#DDE1E8;border-color:#C8D0DA}
[data-theme="light"] .tgl-thumb{box-shadow:0 1px 3px rgba(0,0,0,.15)}
[data-theme="light"] .fmt-radio{border-color:#DDE1E8;background:#FFFFFF}
[data-theme="light"] .preset-card{border-color:#DDE1E8;background:#FFFFFF}
[data-theme="light"] .preset-card:hover{box-shadow:0 4px 12px rgba(0,102,204,.08)}
[data-theme="light"] .dash-card{border-color:#DDE1E8}
[data-theme="light"] .bar-track{background:#E2E6EC;border-color:#DDE1E8}
[data-theme="light"] .col-row{background:#F8F9FB;border-color:#DDE1E8}
[data-theme="light"] .aopt-row{border-color:#DDE1E8;background:#FFFFFF}
[data-theme="light"] .ainp{background:rgba(0,102,204,.03);border-color:rgba(0,102,204,.15)}
[data-theme="light"] .nav-btn:hover{background:#F0F2F5}
[data-theme="light"] .nav-btn.active{background:rgba(0,102,204,.06)}
[data-theme="light"] .nav-btn.logout-btn:hover{background:rgba(239,68,68,.05)}
[data-theme="light"] .theme-switch{background:#DDE1E8;border-color:#C8D0DA}
[data-theme="light"] .pipeline-toggle{border-color:rgba(0,102,204,.3);background:rgba(0,102,204,.04)}
[data-theme="light"] .split-box{background:rgba(245,158,11,.04);border-color:rgba(245,158,11,.2)}
[data-theme="light"] .split-track{background:#DDE1E8}
</style>
</head>
<body>
<div class="layout">
<nav class="sidebar">
  <div class="sidebar-brand">
    <h1><span class="brand-mb">mb</span><span class="brand-fin">finance</span></h1>
    <p>Exportar Leads</p>
  </div>
  <div class="nav-group">
    <span class="nav-label">Exportação</span>
    <button class="nav-btn active" onclick="goPage('filtros',this)"><span class="nav-icon" style="background:var(--accent)"></span>Filtros &amp; Exportar</button>
  </div>
  <div class="nav-group">
    <span class="nav-label">Configuração</span>
    <button class="nav-btn" onclick="goPage('formatos',this)"><span class="nav-icon" style="background:var(--green)"></span>Formatos</button>
    <button class="nav-btn" onclick="goPage('novo-formato',this)"><span class="nav-icon" style="background:var(--warn)"></span>Novo Formato</button>
  </div>
  <div class="nav-group">
    <span class="nav-label">API C6</span>
    <button class="nav-btn" onclick="entrarModoApi(this)"><span class="nav-icon" style="background:var(--warn)"></span>Consulta API</button>
    <button class="nav-btn" onclick="entrarModoApiResultados(this)"><span class="nav-icon" style="background:var(--accent)"></span>Resultados</button>
    <button class="nav-btn" onclick="goPage('teste-fish',this)"><span class="nav-icon" style="background:var(--green)"></span>Teste Fish</button>
  </div>
  <div class="sidebar-footer">
    <div class="sidebar-user">
      <div class="sidebar-avatar">{{ session_user[:1].upper() if session_user else 'U' }}</div>
      <div>
        <div class="sidebar-user-name">{{ session_user or 'Operador' }}</div>
        <div class="sidebar-user-role">Administrador</div>
      </div>
    </div>
    <a href="/logout" style="text-decoration:none">
      <button class="nav-btn logout-btn" type="button">
        <svg style="width:14px;height:14px;flex-shrink:0" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3 3a1 1 0 00-1 1v12a1 1 0 001 1h5a1 1 0 100-2H4V5h4a1 1 0 100-2H3zm11.707 4.293a1 1 0 010 1.414L13.414 10l1.293 1.293a1 1 0 01-1.414 1.414l-2-2a1 1 0 010-1.414l2-2a1 1 0 011.414 0z" clip-rule="evenodd"/><path fill-rule="evenodd" d="M7 10a1 1 0 011-1h8a1 1 0 110 2H8a1 1 0 01-1-1z" clip-rule="evenodd"/></svg>
        Sair
      </button>
    </a>
    <div class="theme-toggle">
      <button class="theme-switch" id="theme-switch" onclick="toggleTheme()" type="button" aria-label="Alternar modo claro">
        <div class="theme-switch-knob">
          <svg class="theme-sun" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clip-rule="evenodd"/></svg>
          <svg class="theme-moon" viewBox="0 0 20 20" fill="currentColor"><path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"/></svg>
        </div>
      </button>
      <span class="theme-toggle-label" id="theme-label">Modo claro</span>
    </div>
    <div class="sidebar-version">MB Finance · v4.2 · Neon DB</div>
  </div>
</nav>
<div class="main">
...
<div id="page-filtros" class="page active">
  ...
  <div style="margin-bottom:28px">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px">
      <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--accent),#4DA3FF);display:flex;align-items:center;justify-content:center;font-size:18px">📤</div>
      <div>
  ...
  <form id="form-export" onsubmit="return false;">
    ...
    <div class="g2" style="margin-bottom:14px;align-items:start">
      ...
      <div class="card">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
              ...
              <div class="pipeline-toggle" onclick="var cb=document.getElementById('limpeza_ativo');cb.checked=!cb.checked;toggleLimpeza(cb)">
                <div class="tgl-switch" style="--tgl-color:var(--accent);width:36px;height:20px;pointer-events:none">
                  <input type="checkbox" name="limpeza_ativo" id="limpeza_ativo" value="sim">
                  <div class="tgl-track"></div>
                  <div class="tgl-thumb" style="top:2px;left:2px;width:16px;height:16px"></div>
                </div>
                <span class="pipeline-label">Ativar pipeline</span>
              </div>
            </div>
            ...
            <div id="limpeza-opts" style="display:none">
              ...
              <!-- 4 opções toggle cards -->
              <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px">
      
                <div class="limp-card" id="opt-raiz" style="--limp-color:var(--accent)" onclick="toggleLimpCard('limpeza_raiz','opt-raiz')">
                  <div class="limp-header">
                    <div class="limp-left">
                      <div class="limp-icon">📋</div>
                      <div>
                        <div class="limp-name">Raiz</div>
                        <div class="limp-step">Etapa 1</div>
                      </div>
                    </div>
                    <label class="tgl-switch" style="--tgl-color:var(--accent)" onclick="event.stopPropagation()">
                      <input type="checkbox" name="limpeza_raiz" id="limpeza_raiz" value="sim" onchange="updateLimpCard('limpeza_raiz','opt-raiz')">
                      <div class="tgl-track"></div>
                      <div class="tgl-thumb"></div>
                    </label>
                  </div>
                  <div class="limp-desc">Remove CNPJs presentes na tabela <code>raiz_cnpjs</code> — evita duplicar clientes já trabalhados</div>
                </div>
      
                <div class="limp-card" id="opt-enrich" style="--limp-color:var(--green)" onclick="toggleLimpCard('limpeza_enrich','opt-enrich')">
                  <div class="limp-header">
                    <div class="limp-left">
                      <div class="limp-icon">📞</div>
                      <div>
                        <div class="limp-name">Enriquecer</div>
                        <div class="limp-step">Etapa 2</div>
                      </div>
                    </div>
                    <label class="tgl-switch" style="--tgl-color:var(--green)" onclick="event.stopPropagation()">
                      <input type="checkbox" name="limpeza_enrich" id="limpeza_enrich" value="sim" onchange="updateLimpCard('limpeza_enrich','opt-enrich')">
                      <div class="tgl-track"></div>
                      <div class="tgl-thumb"></div>
                    </label>
                  </div>
                  <div class="limp-desc">Busca telefones adicionais no banco externo e adiciona nas colunas livres (append)</div>
                </div>
      
                <div class="limp-card" id="opt-fixos" style="--limp-color:var(--warn)" onclick="toggleLimpCard('limpeza_fixos','opt-fixos')">
                  <div class="limp-header">
                    <div class="limp-left">
                      <div class="limp-icon">📵</div>
                      <div>
                        <div class="limp-name">Fixos</div>
                        <div class="limp-step">Etapa 3</div>
                      </div>
                    </div>
                    <label class="tgl-switch" style="--tgl-color:var(--warn)" onclick="event.stopPropagation()">
                      <input type="checkbox" name="limpeza_fixos" id="limpeza_fixos" value="sim" onchange="updateLimpCard('limpeza_fixos','opt-fixos')">
                      <div class="tgl-track"></div>
                      <div class="tgl-thumb"></div>
                    </label>
                  </div>
                  <div class="limp-desc">Remove telefones fixos (3º dígito ≤ 5). A linha permanece, a célula fica vazia</div>
                </div>
      
                <div class="limp-card" id="opt-blocklist" style="--limp-color:var(--danger)" onclick="toggleLimpCard('limpeza_blocklist','opt-blocklist')">
                  <div class="limp-header">
                    <div class="limp-left">
                      <div class="limp-icon">🚫</div>
                      <div>
                        <div class="limp-name">Blocklist</div>
                        <div class="limp-step">Etapa 4</div>
                      </div>
                    </div>
                    <label class="tgl-switch" style="--tgl-color:var(--danger)" onclick="event.stopPropagation()">
                      <input type="checkbox" name="limpeza_blocklist" id="limpeza_blocklist" value="sim" onchange="updateLimpCard('limpeza_blocklist','opt-blocklist')">
                      <div class="tgl-track"></div>
                      <div class="tgl-thumb"></div>
                    </label>
                  </div>
                  <div class="limp-desc">Remove fones que estão na tabela <code>blocklist</code> do banco externo (em lotes de 30k)</div>
                </div>
    ...
    <div class="g2" style="margin-bottom:14px">
      <div class="card">
        ...
        <div id="fmt-dual" style="display:none;flex-direction:column;gap:12px">
          ...
          <div id="aviso-pct" style="display:none;padding:8px 12px;background:var(--warn-l);border:1px solid rgba(245,158,11,.25);border-radius:6px;font-size:12px;color:var(--warn)"></div>
        </div>
      </div>
      <div class="card">
        ...
        <div id="split-options" style="display:none">
          <div style="background:var(--warn-l);border:1px solid rgba(245,158,11,.25);border-radius:var(--radius);padding:12px;margin-bottom:10px">
    ...
    <!-- LIMPAR BLOCKLIST -->
    <div style="margin-top:16px;padding:16px;background:var(--danger-l);border:1px solid #fecaca;border-radius:var(--radius-lg)">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <div>
          <div style="font-size:14px;font-weight:600;color:var(--danger)">🚫 Limpar Blocklist do Banco</div>
          <div style="font-size:12px;color:var(--muted);margin-top:2px">Remove telefones da base de empresas que estejam na blocklist. Mantém o cadastro, apaga só o número.</div>
        </div>
        <button type="button" onclick="iniciarLimpezaBlocklist()" id="btn-limpar-bl" class="btn" style="background:var(--danger);color:#fff;border:none;padding:9px 20px;font-size:13px;white-space:nowrap">🚫 Executar limpeza</button>
      </div>
      <div id="bl-log-panel" style="display:none;margin-top:10px">
        <div id="bl-log-box" style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:10px;font-family:'JetBrains Mono',monospace;font-size:11px;max-height:250px;overflow-y:auto;white-space:pre-wrap;line-height:1.6;color:var(--text)"></div>
      </div>
    </div>
  </form>
</div>
...
<div id="page-novo-formato" class="page">
  ...
  <div class="card" style="padding:14px 16px;background:var(--warn-l);border-color:rgba(245,158,11,.25);margin-bottom:16px">
...
<div id="page-api" class="page">
  ...
  <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:28px">
    <div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
        <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#F59E0B,#FBBF24);display:flex;align-items:center;justify-content:center;font-size:18px">🏦</div>
        <div>
    ...
    <div style="display:flex;align-items:center;gap:8px;padding:6px 12px;background:var(--warn-l);border:1px solid rgba(245,158,11,.25);border-radius:8px;font-size:12px;color:var(--warn)">
      ⛔ MEI sempre excluído
    </div>
  </div>
  ...
  <div class="g2" style="margin-bottom:16px">
    <div class="card">
      ...
      <div style="margin-top:12px">
        <label class="field-label">ORDENAÇÃO POR DATA</label>
        <div class="toggle-group">
          <label class="toggle-opt on"><input type="radio" name="aapi_ordem" value="recentes" checked onchange="updateToggle(this)">Mais recentes</label>
          <label class="toggle-opt"><input type="radio" name="aapi_ordem" value="antigos" onchange="updateToggle(this)">Mais antigos</label>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div style="display:flex;flex-direction:column;gap:8px">
        ...
        <label class="aopt-row">
          <input type="checkbox" id="aapi_excluir_cliente" style="accent-color:var(--danger);width:14px;height:14px;flex-shrink:0" onchange="liveUpdateOpt()">
          <div>
        ...
        <label class="aopt-row">
          <input type="checkbox" id="aapi_extrair_clientes" style="accent-color:#0066CC;width:14px;height:14px;flex-shrink:0">
          <div>
            <div style="font-size:13px;font-weight:500;color:#0066CC">📥 Extrair clientes em Excel</div>
        ...
        <label class="aopt-row" style="border-color:rgba(180,83,9,.2);background:rgba(180,83,9,.03)">
          <input type="checkbox" id="aapi_fish" onchange="document.getElementById('fish-config').style.display=this.checked?'block':'none';liveUpdateOpt()" style="accent-color:#F59E0B;width:14px;height:14px;flex-shrink:0">
          <div>
            <div style="font-size:13px;font-weight:500;color:#F59E0B">🐟 Fish Mode</div>
        ...
        <div id="fish-config" style="display:none;padding:10px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;margin-top:2px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
            ...
            <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--muted);cursor:pointer">
              <input type="checkbox" id="fish_split_ativo" style="accent-color:var(--accent);width:12px;height:12px" onchange="toggleFishSplit(this);liveUpdateOpt()">
              Dividir entre 2
            </label>
          </div>
          <div id="fish-conector-unico">
            <select id="aapi_connector_sel" onchange="toggleConectorCustom(this);liveUpdateOpt()" style="width:100%;margin-bottom:4px">
            ...
            <input id="aapi_connector" type="text" placeholder="Digite o conector..." style="width:100%;display:none" onchange="liveUpdateOpt()">
          </div>
          <div id="fish-conector-split" style="display:none;flex-direction:column;gap:8px">
            ...
            <div class="g2">
              <div>
                ...
                <input id="fish_connector_a" type="text" placeholder="ex: OLOS" style="width:100%;margin-bottom:4px" onchange="liveUpdateOpt()">
                <div style="display:flex;align-items:center;gap:6px">
                  <input id="fish_pct_a" type="number" value="50" min="1" max="99" style="width:70px" oninput="sincronizarPctFish('a');liveUpdateOpt()">
                  <span style="font-size:12px;color:var(--muted)">%</span>
                </div>
              </div>
              <div>
                <label class="field-label">CONECTOR B</label>
                <input id="fish_connector_b" type="text" placeholder="ex: CRM" style="width:100%;margin-bottom:4px" onchange="liveUpdateOpt()">
                <div style="display:flex;align-items:center;gap:6px">
                  <input id="fish_pct_b" type="number" value="50" min="1" max="99" style="width:70px" oninput="sincronizarPctFish('b');liveUpdateOpt()">
                  <span style="font-size:12px;color:var(--muted)">%</span>
                </div>
              </div>
            </div>
  ...
  <div class="card" style="margin-bottom:16px">
    <div class="section-title">⚡ Prioridade de Processamento</div>
    <div style="font-size:12px;color:var(--muted);margin-bottom:14px">Defina a ordem de processamento por natureza. Dentro de cada prioridade, CNPJs mais recentes são processados primeiro.</div>
    <div style="display:flex;gap:12px;align-items:stretch">
      <div style="flex:1;padding:14px;border:2px solid var(--border);border-radius:10px;transition:border-color .15s" id="prio-box-1">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <span style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#F59E0B,#FBBF24);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700">1</span>
          <span style="font-size:12px;font-weight:600;color:var(--warn)">PRIORIDADE 1</span>
        </div>
        <select id="prio_1" onchange="updatePrioBox()" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;background:var(--surface2)">
      ...
      <div style="flex:1;padding:14px;border:2px solid var(--border);border-radius:10px;transition:border-color .15s" id="prio-box-2">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <span style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#F59E0B,#FBBF24);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700">2</span>
          <span style="font-size:12px;font-weight:600;color:#F59E0B">PRIORIDADE 2</span>
        </div>
        <select id="prio_2" onchange="updatePrioBox()" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;background:var(--surface2)">
          <option value="">— Nenhuma —</option>
          {% for cod, nome in naturezas.items() %}
          <option value="{{ cod }}">{{ nome }} ({{ cod }})</option>
          {% endfor %}
      ...
      <div style="flex:1;padding:14px;border:2px solid var(--border);border-radius:10px;transition:border-color .15s" id="prio-box-3">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <span style="width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#64748B,#475569);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700">3</span>
          <span style="font-size:12px;font-weight:600;color:var(--muted)">PRIORIDADE 3</span>
        </div>
        <select id="prio_3" onchange="updatePrioBox()" style="width:100%;padding:8px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;background:var(--surface2)">
          <option value="">— Nenhuma —</option>
          {% for cod, nome in naturezas.items() %}
          <option value="{{ cod }}">{{ nome }} ({{ cod }})</option>
          {% endfor %}
    ...
    <div style="display:flex;gap:6px;margin-top:10px">
      <span class="qtag" onclick="document.getElementById('prio_1').value='2135';document.getElementById('prio_2').value='2305';document.getElementById('prio_3').value='2313';updatePrioBox()">EI + EIRELI</span>
      <span class="qtag" onclick="document.getElementById('prio_1').value='2062';document.getElementById('prio_2').value='2240';document.getElementById('prio_3').value='';updatePrioBox()">Sociedades</span>
      <span class="qtag" onclick="document.getElementById('prio_1').value='';document.getElementById('prio_2').value='';document.getElementById('prio_3').value='';updatePrioBox()">Limpar</span>
    </div>
  </div>
  ...
  <div class="card" style="margin-bottom:16px">
    <div style="display:flex;align-items:center;justify-content:space-between">
      <div class="section-title" style="margin:0">🔁 Agendamento (Repetir)</div>
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer;padding:5px 12px;border:1px solid var(--warn);border-radius:20px;background:var(--warn-l)">
        <input type="checkbox" id="sched_ativo" style="accent-color:var(--warn);width:13px;height:13px" onchange="toggleAgendamento(this)">
        <span style="font-size:12px;font-weight:600;color:var(--warn)">Ativar repetição</span>
      </label>
    </div>
    <div id="sched-opts" style="display:none;margin-top:14px">
      <div class="g3" style="margin-bottom:12px">
        <div>
          <label class="field-label">HORÁRIO DE INÍCIO</label>
          <input id="sched_hora" type="time" value="08:00" style="width:100%">
        </div>
        <div>
          <label class="field-label">INTERVALO (HORAS)</label>
          <div style="display:flex;align-items:center;gap:6px">
            <input id="sched_intervalo" type="number" value="0" min="0" max="24" style="width:70px">
            <span style="font-size:11px;color:var(--muted)">0 = só no horário</span>
          </div>
        </div>
        <div>
          <label class="field-label">STATUS</label>
          <div id="sched-status" style="padding:8px 10px;background:var(--surface2);border:1px solid var(--border);border-radius:6px;font-size:12px;color:var(--muted)">
            Desativado
          </div>
        </div>
      </div>
      <div>
        ...
        <div style="display:flex;gap:6px">
          <label class="toggle-opt on" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="1" checked onchange="updateToggle2(this)">Seg</label>
          <label class="toggle-opt on" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="2" checked onchange="updateToggle2(this)">Ter</label>
          <label class="toggle-opt on" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="3" checked onchange="updateToggle2(this)">Qua</label>
          <label class="toggle-opt on" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="4" checked onchange="updateToggle2(this)">Qui</label>
          <label class="toggle-opt on" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="5" checked onchange="updateToggle2(this)">Sex</label>
          <label class="toggle-opt" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="6" onchange="updateToggle2(this)">Sáb</label>
          <label class="toggle-opt" style="border-radius:6px;padding:6px 12px"><input type="checkbox" class="sched-dia" value="0" onchange="updateToggle2(this)">Dom</label>
        </div>
      </div>
      <div style="margin-top:10px;padding:8px 12px;background:var(--accent-l);border:1px solid var(--accent);border-radius:6px;font-size:12px;color:var(--accent)">
        ℹ O agendamento usa a configuração atual (preset, filtros, chave, fish, etc.). Mantenha a aba aberta.
      </div>
    </div>
  </div>

  <!-- BOTAO INICIAR -->
  <button id="btn-aapi" onclick="iniciarAApi()" class="btn btn-primary btn-lg" style="width:100%;padding:14px;font-size:14px;letter-spacing:.5px;background:linear-gradient(135deg,#F59E0B,#FBBF24);border:none;box-shadow:0 4px 14px rgba(180,83,9,.3);transition:all .2s" onmouseover="this.style.transform='translateY(-1px)';this.style.boxShadow='0 6px 20px rgba(180,83,9,.4)'" onmouseout="this.style.transform='';this.style.boxShadow='0 4px 14px rgba(180,83,9,.3)'">
...
<div id="page-teste-fish" class="page">
  <div style="margin-bottom:24px">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px">
      <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#00A651,#34D399);display:flex;align-items:center;justify-content:center;font-size:18px">🐟</div>
      <div>
...
function toggleLimpCard(cbId, cardId) {
  var cb = document.getElementById(cbId);
  cb.checked = !cb.checked;
  updateLimpCard(cbId, cardId);
}
function updateLimpCard(cbId, cardId) {
  var cb = document.getElementById(cbId);
  var card = document.getElementById(cardId);
  if (cb.checked) { card.classList.add('on'); } else { card.classList.remove('on'); }
}
// Keep backward compat alias
function updateLimpezaOpt(cb, optId) { updateLimpCard(cb.id, optId); }
function limpezaPreset(tipo) {
  ...
  ['raiz','enrich','fixos','blocklist'].forEach(k => {
    ...
    if (cb) { cb.checked = p[k]; updateLimpCard('limpeza_'+k, 'opt-'+k); }
  });
}
// ── Theme toggle ────────────────────────────────────────────
function toggleTheme() {
  ...
  if (html.getAttribute('data-theme') === 'light') {
    ...
    localStorage.setItem('mbf_theme','dark');
  } else {
    html.setAttribute('data-theme','light');
    sw.classList.add('active');
    label.textContent = 'Modo escuro';
    localStorage.setItem('mbf_theme','light');
  }
}
(function(){
  var saved = localStorage.getItem('mbf_theme');
  if (saved === 'light') {
...
async function iniciarExportacao() {
  ...
  exportPolling = setInterval(async () => {
    ...
    if (s.done) {
      ...
      if (s.error) { status.textContent = '❌ Erro'; status.style.color='#EF4444'; }
      else {
        status.textContent = '✅ ' + s.name; status.style.color = '#00A651';
...
function liveUpdateOpt() {
  if (apiJobId) controleApi('update');
}
async function controleApi(action) {
  ...
  if (action === 'update') {
    ...
    body.fish_mode       = document.getElementById('aapi_fish').checked;
    body.excluir_cliente = document.getElementById('aapi_excluir_cliente').checked;
    body.connector       = getConector();
    body.fish_split      = document.getElementById('fish_split_ativo')?.checked || false;
    body.fish_connector_a= document.getElementById('fish_connector_a')?.value.trim() || '';
    body.fish_connector_b= document.getElementById('fish_connector_b')?.value.trim() || '';
    body.fish_pct_a      = parseInt(document.getElementById('fish_pct_a')?.value||50);
  }
  try {
...
async function iniciarAApi() {
  ...
  const body = {
    ...
    priority_naturezas:   getPrioNat(),
    ordem:               document.querySelector('input[name="aapi_ordem"]:checked')?.value || 'recentes',
  };
...
async function carregarResultados() {
  ...
  drawDonut('chart-cobertura','legend-cobertura',[
    {label:'Processados',value:total,color:'#0066CC'},
    {label:'Não processados',value:naoProc,color:'#1E3044'},
  ]);
  drawDonut('chart-resultado','legend-resultado',[
    {label:'Disponível',value:disp,color:'#00A651'},
    {label:'Cliente C6',value:cli,color:'#EF4444'},
  ]);
  const barsEl=document.getElementById('dash-bars');
  const bars=[
    {label:'Cobertura da base',value:pctProc,color:'#0066CC',fmt:`${total.toLocaleString('pt-BR')} / ${totalBd.toLocaleString('pt-BR')}`},
    {label:'Taxa de disponibilidade',value:pctDisp,color:'#00A651',fmt:`${disp.toLocaleString('pt-BR')} disponíveis`},
    {label:'Taxa de clientes C6',value:pctCli,color:'#EF4444',fmt:`${cli.toLocaleString('pt-BR')} clientes`},
  ];
...
function drawDonut(canvasId,legendId,segments) {
  ...
  if (total===0) { ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2); ctx.strokeStyle=document.documentElement.getAttribute('data-theme')==='light'?'#DDE1E8':'#1E3044'; ctx.lineWidth=r-inner; ctx.stroke(); }
  else {
    let start=-Math.PI/2;
  ...
  ctx.fillStyle=document.documentElement.getAttribute('data-theme')==='light'?'#0F172A':'#E5E7EB'; ctx.font='bold 18px Sora,sans-serif';
  ...
  if (!fones.length) {
    ...
    document.getElementById('teste-fones').innerHTML = fones.map((f,i) =>
      `<div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid var(--border)">
        ...
        ${(foneSrc==='externo')?'<span style="font-size:10px;padding:2px 8px;background:var(--warn-l);color:var(--warn);border-radius:20px;border:1px solid rgba(245,158,11,.25)">banco externo</span>':'<span style="font-size:10px;padding:2px 8px;background:var(--green-l);color:var(--green);border-radius:20px">empresa</span>'}
      </div>`
...
// ── Limpar Blocklist do Banco ────────────────────────────────
let blPolling = null;
async function iniciarLimpezaBlocklist() {
  if (!confirm('Isso vai limpar os telefones da base que estão na blocklist. Continuar?')) return;
  const btn = document.getElementById('btn-limpar-bl');
  const panel = document.getElementById('bl-log-panel');
  const box = document.getElementById('bl-log-box');
  btn.disabled = true; btn.textContent = '⏳ Processando...';
  panel.style.display = 'block'; box.textContent = '';
  try {
    const resp = await fetch('/api/limpar-blocklist', {method:'POST'});
    const json = await resp.json();
    const jobId = json.job_id;
    blPolling = setInterval(async () => {
      const r = await fetch('/api/limpar-blocklist-status/' + jobId);
      const s = await r.json();
      box.textContent = s.log.join('\n');
      box.scrollTop = box.scrollHeight;
      if (s.done) {
        clearInterval(blPolling); blPolling = null;
        btn.disabled = false; btn.textContent = '🚫 Executar limpeza';
      }
    }, 1500);
  } catch(e) {
...
// ── Prioridade Natureza ──────────────────────────────────────
// ── Prioridade Natureza (ranking 1-2-3) ─────────────────────
function updatePrioBox() {
  const colors = ['var(--warn)','#F59E0B','var(--muted2)'];
  const bgs = ['var(--warn-l)','rgba(146,64,14,.06)',''];
  for (let i = 1; i <= 3; i++) {
    const box = document.getElementById('prio-box-'+i);
    const val = document.getElementById('prio_'+i).value;
    box.style.borderColor = val ? colors[i-1] : 'var(--border)';
    box.style.background  = val ? bgs[i-1] : '';
  }
}
function getPrioNat() {
  const prios = [];
  for (let i = 1; i <= 3; i++) {
    const v = document.getElementById('prio_'+i)?.value;
    if (v) prios.push(v);
  }
  return prios;
}
...
function toggleAgendamento(cb) {
  document.getElementById('sched-opts').style.display = cb.checked ? 'block' : 'none';
  if (cb.checked) {
    iniciarAgendamento();
  } else {
    pararAgendamento();
...
function updateToggle2(el) {
  el.closest('label').classList.toggle('on', el.checked);
}
function iniciarAgendamento() {
  if (schedInterval) clearInterval(schedInterval);
  schedRunning = false;
  atualizarStatusSched();
  schedInterval = setInterval(verificarAgendamento, 30000); // checa a cada 30s
  verificarAgendamento(); // checa imediatamente
}
function pararAgendamento() {
  if (schedInterval) { clearInterval(schedInterval); schedInterval = null; }
  schedRunning = false;
  document.getElementById('sched-status').textContent = 'Desativado';
  document.getElementById('sched-status').style.color = 'var(--muted)';
}
function atualizarStatusSched() {
  const hora = document.getElementById('sched_hora').value;
  const dias = [...document.querySelectorAll('.sched-dia:checked')].map(c => ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][c.value]);
  const el = document.getElementById('sched-status');
  if (schedRunning) {
    el.innerHTML = '<span style="color:var(--green);font-weight:600">🟢 Executando...</span>';
  } else {
    el.innerHTML = `⏰ Próximo: <strong>${hora}</strong> — ${dias.join(', ')||'nenhum dia'}`;
    el.style.color = 'var(--warn)';
  }
}
function verificarAgendamento() {
  if (!document.getElementById('sched_ativo')?.checked) return;
  if (schedRunning) return;
  // Não iniciar se já há job ativo
  if (apiJobId && apiPolling) return;

  const agora = new Date();
  const diaAtual = agora.getDay(); // 0=dom
  const diasAtivos = [...document.querySelectorAll('.sched-dia:checked')].map(c => parseInt(c.value));
  if (!diasAtivos.includes(diaAtual)) { atualizarStatusSched(); return; }

  const horaConfig = document.getElementById('sched_hora').value; // "HH:MM"
  const [hh, mm] = horaConfig.split(':').map(Number);
  const intervalo = parseInt(document.getElementById('sched_intervalo').value) || 0;

  const minAtual = agora.getHours() * 60 + agora.getMinutes();
  const minConfig = hh * 60 + mm;

  let deveRodar = false;
  if (intervalo > 0) {
    // Checa se estamos em algum slot de intervalo a partir do horário base
    const diffMin = minAtual - minConfig;
    if (diffMin >= 0 && diffMin % (intervalo * 60) < 1) deveRodar = true;
  } else {
    // Só no horário exato (janela de 1 min)
    if (Math.abs(minAtual - minConfig) < 1) deveRodar = true;
  }

  if (deveRodar) {
    schedRunning = true;
    atualizarStatusSched();
    aApiLog('🔁 AGENDAMENTO: iniciando execução automática...', '#ffaa22');
    iniciarAApi();
    // Quando o job terminar, reseta schedRunning
...
app = Flask(__name__)
app.secret_key = 'mbf_leads_secret_key_2026_xK9pL3'

USERS = {
    "Davi": {"senha": "080472Fr*", "role": "adm"},
}
...
<style>
:root{
  --blue:#0066CC;--blue-dark:#004FA3;--blue-light:#E8F2FF;--blue-mid:#CCE0FF;
  --green:#00A651;--green-light:#E6F7EE;--green-dark:#007A3D;
  --gray-50:#F8F9FB;--gray-100:#F0F2F5;--gray-200:#E2E6EC;--gray-300:#C8D0DA;
  --gray-400:#98A4B3;--gray-500:#64748B;--gray-600:#475569;--gray-700:#334155;--gray-900:#0F172A;
  --white:#FFFFFF;
  --danger-bg:#FFF1F1;--danger-border:#EF4444;--danger-text:#7F1D1D;
  --font:'Sora',sans-serif;--mono:'DM Mono',monospace;
}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
body{font-family:var(--font);background:#080B14;min-height:100vh;overflow:hidden}
...
.scan-line{
  ...
  background:linear-gradient(180deg,transparent 0%,rgba(0,102,204,.03) 50%,transparent 100%);
...
.login-card{
  ...
  box-shadow:0 0 80px rgba(0,102,204,.08),0 20px 60px rgba(0,0,0,.4);
...
.login-card.shrink{
  animation:cardShrink .3s cubic-bezier(.16,1,.3,1) forwards;
}
@keyframes cardIn{
...
.login-logo-text{
  ...
  line-height:1;display:inline-flex;overflow:hidden;
}
.logo-letter{
  display:inline-block;opacity:0;
}
.logo-letter.show{
  animation:letterIn .25s cubic-bezier(.16,1,.3,1) forwards;
}
@keyframes letterIn{
...
.login-logo-sub{
  ...
  color:var(--gray-400);margin-top:7px;opacity:0;
}
.login-logo-sub.animate{
  animation:subSlide .4s cubic-bezier(.16,1,.3,1) forwards;
}
@keyframes subSlide{
...
/* ─── DIVIDER ─── */
.login-divider{
  height:1px;margin:0 0 24px;width:0;opacity:0;
  background:linear-gradient(90deg,transparent,rgba(0,102,204,.4),transparent);
}
.login-divider.animate{animation:divGrow .3s ease forwards}
@keyframes divGrow{from{width:0;opacity:0}to{width:100%;opacity:1}}

/* ─── TITLES ─── */
.login-title{
  font-size:16px;font-weight:700;color:#E5E7EB;margin-bottom:3px;opacity:0;
}
.login-sub{
  font-size:12px;color:var(--gray-400);margin-bottom:24px;opacity:0;
}
.login-title.animate,.login-sub.animate{
  animation:elIn .3s cubic-bezier(.16,1,.3,1) forwards;
}
@keyframes elIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}

/* ─── FIELDS ─── */
.login-field{display:flex;flex-direction:column;gap:5px;margin-bottom:14px;opacity:0}
.login-field.animate{animation:elIn .25s cubic-bezier(.16,1,.3,1) forwards}
.login-field label{font-size:11px;font-weight:600;color:var(--gray-400);letter-spacing:.04em;text-transform:uppercase}
.login-input-wrap{position:relative;display:flex;align-items:center}
.login-input-wrap .l-icon{
...
.login-input-wrap input{
  ...
  border-radius:8px;border:1.5px solid rgba(0,102,204,.2);
  background:rgba(15,23,42,.6);font-size:13px;font-family:var(--font);
  color:#E5E7EB;outline:none;
  transition:border-color .18s,box-shadow .18s,background .18s;
}
.login-input-wrap input::placeholder{color:var(--gray-500)}
.login-input-wrap input:focus{
  border-color:var(--blue);background:rgba(0,102,204,.06);
  box-shadow:0 0 0 3px rgba(0,102,204,.15),0 0 20px rgba(0,102,204,.05);
}
.login-input-wrap .eye-btn{
  position:absolute;right:12px;top:50%;transform:translateY(-50%);
  background:none;border:none;cursor:pointer;padding:4px;
  color:var(--gray-400);display:flex;align-items:center;border-radius:4px;transition:color .15s;
}
.login-input-wrap .eye-btn:hover{color:var(--blue)}

/* ─── ERROR ─── */
.login-error{
  background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.25);
  color:#FCA5A5;border-radius:8px;padding:9px 13px;font-size:12px;
  margin-bottom:14px;display:none;align-items:center;gap:8px;
}

/* ─── REMEMBER ─── */
.login-remember{display:flex;align-items:center;margin:2px 0 6px;opacity:0}
.login-remember.animate{animation:elIn .25s cubic-bezier(.16,1,.3,1) forwards}
.remember-label{
  display:flex;align-items:center;gap:8px;font-size:12px;font-weight:500;
  color:var(--gray-400);cursor:pointer;user-select:none;transition:color .15s;
}
.remember-label:hover{color:var(--gray-300)}
.remember-label input{position:absolute;opacity:0;width:0;height:0;pointer-events:none}
.remember-check{
  width:18px;height:18px;border-radius:5px;
  border:1.5px solid rgba(0,102,204,.3);background:rgba(0,102,204,.06);
  display:flex;align-items:center;justify-content:center;
...
.login-btn{
  ...
  transition:all .18s;margin-top:8px;opacity:0;
  position:relative;overflow:hidden;
}
.login-btn.animate{animation:elIn .3s cubic-bezier(.16,1,.3,1) forwards}
.login-btn:hover{
  background:var(--blue-dark);
  box-shadow:0 0 30px rgba(0,102,204,.3),0 6px 20px rgba(0,102,204,.25);
...
/* ─── FOOTER ─── */
.login-footer{text-align:center;margin-top:20px;font-size:10px;color:rgba(100,116,139,.5);opacity:0}
.login-footer.animate{animation:elIn .3s cubic-bezier(.16,1,.3,1) forwards}

/* ─── CORNER ACCENTS ─── */
.corner-accent{position:fixed;z-index:3;pointer-events:none}
.corner-tl{top:24px;left:24px;color:rgba(0,102,204,.15);font-family:var(--mono);font-size:10px;line-height:1.8}
.corner-br{bottom:24px;right:24px;color:rgba(0,102,204,.15);font-family:var(--mono);font-size:10px;text-align:right;line-height:1.8}
.corner-stat{color:rgba(0,166,81,.2)}
.pulse-dot{display:inline-block;width:5px;height:5px;background:var(--green);border-radius:50%;margin-right:4px;animation:pulseDot 2s ease infinite}
@keyframes pulseDot{0%,100%{opacity:1;box-shadow:0 0 0 0 rgba(0,166,81,.4)}50%{box-shadow:0 0 0 6px rgba(0,166,81,0)}}

@media(max-width:480px){.login-card{padding:32px 24px 28px}.login-logo-text{font-size:26px}}
</style>
</head>
<body>
...
<div class="corner-accent corner-tl">
  SYS::LEAD_EXPORT<br>
  <span class="corner-stat"><span class="pulse-dot"></span>ONLINE</span><br>
  v4.2.0 // NEON_DB
</div>
<div class="corner-accent corner-br">
  MB FINANCE CORP<br>
  BASE RECEITA FEDERAL<br>
  12M+ REGISTROS
</div>
  ...
  <div class="login-card" id="login-card">
    <div class="login-logo">
      <div class="login-logo-text" id="login-logo-text">
        <span class="logo-letter" style="color:#0066CC">m</span><span class="logo-letter" style="color:#0066CC">b</span><span class="logo-letter" style="color:#E5E7EB">f</span><span class="logo-letter" style="color:#E5E7EB">i</span><span class="logo-letter" style="color:#E5E7EB">n</span><span class="logo-letter" style="color:#E5E7EB">a</span><span class="logo-letter" style="color:#E5E7EB">n</span><span class="logo-letter" style="color:#E5E7EB">c</span><span class="logo-letter" style="color:#E5E7EB">e</span>
      </div>
      <div class="login-logo-sub" id="login-logo-sub">Exportar Leads</div>
    </div>
    <div class="login-divider" id="login-divider"></div>
    <div class="login-title" id="login-title">Acesso ao sistema</div>
    <div class="login-sub" id="login-sub">Credenciais de operador autorizadas</div>

    <div class="login-error" id="login-error">
      <svg width="14" height="14" viewBox="0 0 20 20" fill="#FCA5A5"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>
      <span id="login-error-text">Usuário ou senha incorretos.</span>
    </div>

    <form method="POST" action="/login" id="login-form">
    <div class="login-field" id="login-field-user">
      <label>Usuário</label>
      <div class="login-input-wrap">
        <span class="l-icon"><svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"/></svg></span>
        <input type="text" name="username" id="login-user" placeholder="Usuário" autocomplete="username">
      </div>
    </div>

    <div class="login-field" id="login-field-pass">
      <label>Senha</label>
      <div class="login-input-wrap">
        <span class="l-icon"><svg width="16" height="16" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"/></svg></span>
        <input type="password" name="password" id="login-pass" placeholder="Senha de acesso" autocomplete="current-password">
        <button class="eye-btn" type="button" onclick="toggleSenha()" aria-label="Mostrar senha">
          <svg id="eye-icon" width="17" height="17" viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z"/><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd"/></svg>
        </button>
      </div>
    </div>

    <div class="login-remember" id="login-remember">
      <label class="remember-label" for="login-keep">
        <input type="checkbox" name="remember" id="login-keep" value="1">
        <span class="remember-check"><svg width="10" height="10" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2.5 6l2.5 2.5 4.5-5"/></svg></span>
        Manter conectado
      </label>
    </div>

    <button type="submit" class="login-btn" id="login-btn-submit">Acessar Sistema</button>
    </form>

    <div class="login-footer" id="login-footer">MB Finance · Exportar Leads · Base Receita Federal</div>
  </div>
</div>
...
(function(){
  ...
  for(var i=0;i<30;i++){
    ...
    dot.style.opacity=String(Math.random()*.4+.1);
    p.appendChild(dot);
  }
})();
...
// ── Build login animation ──
(function(){
  var card=document.getElementById('login-card');
  var letters=document.querySelectorAll('#login-logo-text .logo-letter');
  var sub=document.getElementById('login-logo-sub');
  ...
  var fieldUser=document.getElementById('login-field-user');
  var fieldPass=document.getElementById('login-field-pass');
  ...
  setTimeout(function(){card.classList.add('animate')},100);
  var ld=400;
  letters.forEach(function(l,i){setTimeout(function(){l.classList.add('show')},ld+(i*50))});
  var af=ld+(letters.length*50)+100;
  setTimeout(function(){sub.classList.add('animate')},af);
  setTimeout(function(){divider.classList.add('animate')},af+200);
  setTimeout(function(){title.classList.add('animate')},af+350);
  setTimeout(function(){loginSub.classList.add('animate')},af+430);
  setTimeout(function(){fieldUser.classList.add('animate')},af+550);
  setTimeout(function(){fieldPass.classList.add('animate')},af+650);
  setTimeout(function(){remember.classList.add('animate')},af+750);
  setTimeout(function(){btn.classList.add('animate')},af+850);
  setTimeout(function(){footer.classList.add('animate')},af+950);
  setTimeout(function(){card.style.opacity='1';card.classList.add('shrink')},af+980);
  setTimeout(function(){document.getElementById('login-user').focus()},af+1000);
})();

// ── Show error if present ──
{% if erro %}
document.getElementById('login-error').style.display='flex';
document.getElementById('login-error-text').textContent='{{ erro }}';
{% endif %}
</script>
</body>
</html>"""
...
def _fish_enviar(cnpj, row, connector, job_id):
    """
    Envia cliente ao N8N com retry (3 tentativas silenciosas).
    - Aplica inserir9 em todos os telefones
    - Se não tiver telefone na empresa, busca no banco externo
    - Se não tiver nenhum telefone, NÃO envia
    """
    ...
    for attempt in range(1, 4):
        try:
            ...
            if attempt == 3:
                _api_log(job_id, f"  🐟 FISH ERRO (3 tentativas): {cnpj_norm}")
                return False
            time.sleep(2)
    return False
...
def _executar_api(job_id, params):
    try:
        ...
        priority_naturezas = params.get("priority_naturezas", [])
        ordem = params.get("ordem", "recentes")  # recentes = DESC, antigos = ASC
        ...
        # Sempre seleciona colunas completas — permite ligar fish em tempo real
        select_cols = "cnpj,razao_social,telefone_principal,telefone_secundario,email"
        ...
        total_disp = total_cli = total_fish = total_excluidos = 0
        MAX_RETRY  = 5
        lote_num   = 0
        offset     = 0  # tracks position — new connection each batch, no long-lived cursor

        # Monta ORDER BY: prioridades ranqueadas (1→2→3→restante), ordem por data
        order_dir = "DESC" if ordem == "recentes" else "ASC"
        ordem_label = "mais recentes primeiro" if ordem == "recentes" else "mais antigos primeiro"
        order_by = f" ORDER BY data_abertura {order_dir}"
        if priority_naturezas:
            cases = []
            for i, nat in enumerate(priority_naturezas):
                cases.append(f"WHEN natureza_juridica_cod = '{nat}' THEN {i}")
            case_sql = "CASE " + " ".join(cases) + f" ELSE {len(priority_naturezas)} END"
            order_by = f" ORDER BY {case_sql}, data_abertura {order_dir}"
            nomes_nat = {n: NATUREZAS.get(n, n) for n in priority_naturezas}
            for i, nat in enumerate(priority_naturezas, 1):
                _api_log(job_id, f"⚡ Prioridade {i}: {nomes_nat[nat]} ({nat}) — {ordem_label}")
            _api_log(job_id, f"   Restante processa após as prioridades ({ordem_label})")
        else:
            _api_log(job_id, f"📅 Sem prioridade definida — {ordem_label}")

        stream_sql = f"SELECT {select_cols} FROM empresas WHERE {where_sql_count}{order_by}"
        ...
        while True:
            ...
            cur_fish    = live.get("fish_mode", fish_mode)
            cur_excluir = live.get("excluir_cliente", excluir_cliente)
            cur_connector       = live.get("connector", connector)
            cur_fish_split      = live.get("fish_split", fish_split)
            cur_fish_conn_a     = live.get("fish_connector_a", fish_connector_a)
            cur_fish_conn_b     = live.get("fish_connector_b", fish_connector_b)
            cur_fish_pct_a      = live.get("fish_pct_a", fish_pct_a)
            ...
            row_map   = {(r["cnpj"] if isinstance(r,dict) else r[0]): r for r in rows}
            ...
            if cur_fish:
                ...
                for r in resultados:
                    if r["status"] == "cliente":
                        if cur_fish_split and cur_fish_conn_a and cur_fish_conn_b:
                            cnt_a = fish_dist.get(cur_fish_conn_a, 0)
                            cnt_b = fish_dist.get(cur_fish_conn_b, 0)
                            total_ab = cnt_a + cnt_b
                            atual_a  = cnt_a / max(total_ab, 1)
                            target_a = cur_fish_pct_a / 100
                            conn_usar = cur_fish_conn_a if atual_a <= target_a else cur_fish_conn_b
                        else:
                            conn_usar = cur_connector
            ...
            # ── Excluir clientes DESTE lote (após fish) ──────
            if cur_excluir and cli > 0:
                clientes_lote = [r["cnpj"] for r in resultados if r["status"] == "cliente"]
                if clientes_lote:
                    try:
                        conn_del = get_conn(); cur_del = conn_del.cursor()
                        cur_del.execute("DELETE FROM empresas WHERE cnpj = ANY(%s)", (clientes_lote,))
                        del_count = cur_del.rowcount; conn_del.commit(); cur_del.close(); conn_del.close()
                        total_excluidos += del_count
                        _api_log(job_id, f"  🗑 {del_count:,} clientes removidos (total: {total_excluidos:,})")
                    except Exception as e:
                        _api_log(job_id, f"  ⚠ Erro ao excluir lote: {e}")
        ...
        if total_excluidos:
            _api_log(job_id, f"🗑 Total excluídos do banco: {total_excluidos:,} clientes (por lote)")

        # Libera chaves
        _api_log(job_id, f"🔓 Chave(s) liberadas: {key_mode}")
        ...
        if extrair_clientes and total_cli > 0:
            ...
            try:
                ...
                cur3.execute("""SELECT l.cnpj,e.razao_social,e.telefone_principal,e.telefone_secundario,
                    e.email,e.estado,e.cidade,e.atividade_principal_cod,l.processado_em
                    FROM limpeza_api l LEFT JOIN empresas e ON e.cnpj=l.cnpj
...
def api_controle(job_id):
    ...
    if   action == "pause":  live["paused"] = True
        ...
        # Fish on/off
        if "fish_mode" in data:
            old = live.get("fish_mode")
            live["fish_mode"] = data["fish_mode"]
            if old is not None and old != data["fish_mode"]:
                changes.append(f"🐟 Fish {'LIGADO' if data['fish_mode'] else 'DESLIGADO'}")
        # Excluir on/off
        if "excluir_cliente" in data:
            old = live.get("excluir_cliente")
            live["excluir_cliente"] = data["excluir_cliente"]
            if old is not None and old != data["excluir_cliente"]:
                changes.append(f"🗑 Excluir {'LIGADO' if data['excluir_cliente'] else 'DESLIGADO'}")
        # Conector único
        if "connector" in data:
            old = live.get("connector")
            live["connector"] = data["connector"]
            if old is not None and old != data["connector"]:
                changes.append(f"🔌 Conector → {data['connector'] or 'sem conector'}")
        # Split conector
        if "fish_split" in data:
            old = live.get("fish_split")
            live["fish_split"] = data["fish_split"]
            if old is not None and old != data["fish_split"]:
                changes.append(f"✂ Split {'LIGADO' if data['fish_split'] else 'DESLIGADO'}")
        for k in ["fish_connector_a","fish_connector_b"]:
            if k in data:
                old = live.get(k)
                live[k] = data[k]
                if old is not None and old != data[k] and data[k]:
                    label = "A" if "a" in k else "B"
                    changes.append(f"🔌 Conector {label} → {data[k]}")
        if "fish_pct_a" in data:
            old = live.get("fish_pct_a")
            live["fish_pct_a"] = int(data["fish_pct_a"])
            if old is not None and old != int(data["fish_pct_a"]):
                changes.append(f"📊 Split: {data['fish_pct_a']}% / {100-int(data['fish_pct_a'])}%")
        if changes:
            for msg in changes:
                _api_log(job_id, f"  ⚙ LIVE: {msg}")
...
_bl_jobs = {}  # job_id → {"log": [], "done": bool}

@app.route('/api/limpar-blocklist', methods=['POST'])
@login_required
def api_limpar_blocklist():
    from flask import jsonify
    job_id = str(uuid.uuid4())[:8]
    _bl_jobs[job_id] = {"log": [], "done": False}
    t = threading.Thread(target=_executar_limpeza_blocklist, args=(job_id,))
    t.daemon = True
    t.start()
    return jsonify({"job_id": job_id})

@app.route('/api/limpar-blocklist-status/<job_id>')
@login_required
def api_limpar_blocklist_status(job_id):
    from flask import jsonify
    job = _bl_jobs.get(job_id, {"log": [], "done": True})
    return jsonify({"log": job["log"], "done": job["done"]})

def _executar_limpeza_blocklist(job_id):
    def _log(msg):
        _bl_jobs[job_id]["log"].append(msg)
    try:
        _log("🚫 Iniciando limpeza de blocklist no banco de empresas...")
        _log("")

        # 1. Carrega blocklist do banco externo
        _log("📥 Carregando blocklist do banco externo...")
        t0 = time.time()
        conn_ext = get_ext_conn()
        cur_ext = conn_ext.cursor()
        cur_ext.execute("SELECT telefone FROM blocklist")
        blocklist = {r[0] for r in cur_ext.fetchall()}
        cur_ext.close(); conn_ext.close()
        _log(f"   ✅ {len(blocklist):,} telefones na blocklist ({time.time()-t0:.1f}s)")
        _log("")

        if not blocklist:
            _log("⚠ Blocklist vazia — nada a fazer.")
            _bl_jobs[job_id]["done"] = True
            return

        BATCH = 50_000
        blocklist_list = list(blocklist)
        total_lotes = (len(blocklist_list) + BATCH - 1) // BATCH
        total_principal = 0
        total_secundario = 0

        def _executar_lote_update(campo, lote, lote_num):
            """Executa UPDATE com retry 3x."""
            for attempt in range(1, 4):
                try:
                    ...
                    cur.execute(f"""
                        UPDATE empresas
                        SET {campo} = NULL
                        WHERE regexp_replace({campo}, '\\D', '', 'g') = ANY(%s)
                    ...
                    if attempt == 3:
                        _log(f"   ⚠ Lote {lote_num}/{total_lotes} falhou após 3 tentativas: {e}")
                        return 0
                    time.sleep(3)
            return 0

        # 2. telefone_principal
        _log(f"🔍 Limpando telefone_principal — {total_lotes} lotes de {BATCH:,}...")
        for i in range(0, len(blocklist_list), BATCH):
            lote = blocklist_list[i:i+BATCH]
            lote_num = i // BATCH + 1
            count = _executar_lote_update("telefone_principal", lote, lote_num)
            total_principal += count
            _log(f"   📞 Lote {lote_num}/{total_lotes}: {count:,} limpos (acum: {total_principal:,})")

        _log("")

        # 3. telefone_secundario
        _log(f"🔍 Limpando telefone_secundario — {total_lotes} lotes de {BATCH:,}...")
        for i in range(0, len(blocklist_list), BATCH):
            lote = blocklist_list[i:i+BATCH]
            lote_num = i // BATCH + 1
            count = _executar_lote_update("telefone_secundario", lote, lote_num)
            total_secundario += count
            _log(f"   📞 Lote {lote_num}/{total_lotes}: {count:,} limpos (acum: {total_secundario:,})")

        _log("")
        _log("─" * 50)
        _log(f"🎉 Limpeza concluída!")
        _log(f"   📞 Telefones principais limpos: {total_principal:,}")
        _log(f"   📞 Telefones secundários limpos: {total_secundario:,}")
        _log(f"   📊 Total de campos limpos: {total_principal + total_secundario:,}")
        _bl_jobs[job_id]["done"] = True
